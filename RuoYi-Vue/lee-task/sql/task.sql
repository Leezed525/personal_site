CREATE TABLE `lee_task_definition` IF NOT EXISTS
(
...
CREATE TABLE `lee_task_instance` IF NOT EXISTS
(
    `id`              VARCHAR(64) NOT NULL COMMENT '主键ID(UUID)',
    `definition_id`   VARCHAR(64) NOT NULL COMMENT '任务定义ID',
    `trigger_type`    VARCHAR(16) NOT NULL COMMENT '触发类型: SCHEDULED/MANUAL/API',
    `status`          VARCHAR(32) NOT NULL DEFAULT 'PENDING' COMMENT '实例状态',
    `executor_result` TEXT NULL COMMENT '执行结果(JSON)',
    `pre_op_result`   TEXT NULL COMMENT '前置处理结果(JSON)',
    `post_op_result`  TEXT NULL COMMENT '后置处理结果(JSON)',
    `exception_info`  TEXT NULL COMMENT '异常详情',
    `retry_count`     INT         NOT NULL DEFAULT 0 COMMENT '重试次数',
    `start_time`      DATETIME NULL COMMENT '开始时间',
    `end_time`        DATETIME NULL COMMENT '结束时间',
    `duration_ms`     BIGINT NULL COMMENT '耗时(毫秒)',
    `triggered_by`    VARCHAR(128) NULL COMMENT '触发者',
    `log_file_path`   VARCHAR(512) NULL COMMENT '日志文件路径',
    `create_time`     DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
    KEY               `idx_definition_id_status` (`definition_id`, `status`),
